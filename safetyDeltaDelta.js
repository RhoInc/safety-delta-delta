(function (global, factory) {
    ...
    function initSettings() {
        ...
        this.config.visits.baseline = this.config.visits.baseline.length > 0 ? this.config.visits.baseline : [this.visits[0]];

        this.config.visits.comparison = this.config.visits.comparison.length > 0 ? this.config.visits.comparison : [this.visits[this.visits.length - 1]];
    }
    ...
    function addRegressionLine() {
        if (this.config.addRegressionLine) {
            ...
            chart.wrap.append('span').attr('class', 'regressionNote').style('font-size', '0.8em').style('color', '#999').html('The dashed line shows the result of a simple linear regression. Additional details are shown below. <br> Equation: ' + result.string + '<br> R<sup>2</sup>: ' + d3.format('0.2f')(result.r2));
        }
    }
    ...
    function defineStyles() {
        ...
        //controls
        '#sdd-controls {' + '    width: 25%;' + '    float: left;' + '}', '#sdd-controls .control-group {' + '    width: 98%;' + '    margin: 0 2% 5px 0;' + '    padding: 0;' + '}', '#sdd-controls .control-group > * {' + '    display: inline-block;' + '}', '#sdd-controls .changer {' + '    float: right;' + '    width: 50%;' + '}', '#sdd-controls .wc-control-label {' + '    text-align: right;' + '    width: 48%;' + '}', '#sdd-controls .annote {' + '    width: 98%;' + '    text-align: right;' + '}',
